...
def make_MILP_length(A, k, gateXings_constraint=False, gates_limit=False,
    ...
    m.De = pyo.Var(m.diE, domain=pyo.NonNegativeReals,
    ...
    # Ne = len(E)
    # sub_idx = tuple(zip(range(Ne), range(Ne, 2*Ne)))
    # m.cons_one_diEdge = pyo.SOSConstraint(sub_idx, var=m.Be, index=m.diE, sos=1)
    m.cons_one_diEdge = pyo.SOSConstraint(
        E,
        rule=lambda m, u, v: [m.Be[u, v], m.Be[v, u]],
        sos=1
    )
    ...
    # idx = pyo.Set(m.R, initialize=(((r, n) for r in m.R) for n in m.N))
    # m.cons_one_root = pyo.SOSConstraint(m.R, var=m.Bg, index=idx, sos=1)
    m.cons_one_root = pyo.SOSConstraint(
        m.N,
        rule=lambda m, n: list(m.Bg[:, n]),
        sos=1
    )
    ...
    if gateXings_constraint:
        m.cons_gateXedge = pyo.SOSConstraint(
            gateXing_iter(A),
            rule=lambda m, u, v, r, n: [m.Be[u, v], m.Be[v, u], m.Bg[r, n]],
            sos=1
        )
    ...
    if doubleXings:
        m.cons_edgeXedge = pyo.SOSConstraint(
            doubleXings, rule=edgeXedge_rule, sos=1)
        # m.cons_edgeXedge = pyo.Constraint(doubleXings,
        #                                   rule=edgeXedge_rule)
    if tripleXings:
        m.cons_edgeXedgeXedge = pyo.SOSConstraint(
            tripleXings, rule=edgeXedge_rule, sos=1)
    ...
    if not branching:
        ...
        m.non_branching = pyo.SOSConstraint(
            m.N,
            rule=lambda m, u: list(m.Be[v, u] for v in A_nodes.neighbors(u)),
            sos=1
        )
