...
'''
Attempt to make an ILP implementation of a BCP_k (balanced connected
partition) of the A graph. This is failing because the adjacency matrix Ba
is not respecting actual available graph paths. Not sure if this is fixable.
'''
...
def make_MILP_length(A, κ, gateXings_constraint=False, gates_limit=False,
    ...
    m.node2pairs_map = {n: tuple((v, n) for v in range(0, n))
    ...
    #  m.Bg = pyo.Var(m.N, domain=pyo.Binary, initialize=0)

    # row is the node, column is the gate that is used by the node
    m.Bng = pyo.Var(m.N, m.N, domain=pyo.Binary, initialize=0)

    m.Bg = {n: m.Bng[n, n] for n in range(N)}

    # degree of nodes. TODO: remove, this proved to be useless
    #  m.degree = pyo.Var(m.N, domain=pyo.PositiveIntegers, initialize=1)
    ...
    # link m.degree and m.Be
    #  m.cons_degree_Be = pyo.Constraint(
    #      m.N,
    #      rule=lambda m, n: (sum(m.Be[(n, nn) if n < nn else (nn, n)]
    #                             for nn in A_nodes.neighbors(n))
    #                         + m.Bg[n] == m.degree[n]))
    #  m.cons_degree_total = pyo.Constraint(expr=(sum(m.degree.values())
    #                                             == 2*N - math.ceil(N/κ)))

    # all nodes take part in at least one edge
    m.cons_node_has_edge = pyo.Constraint(
        m.N,
        rule=lambda m, n: (sum(m.Be[(n, nn) if n < nn else (nn, n)]
                               for nn in A_nodes.neighbors(n))
                           + m.Bg[n] >= 1))
    # each node's component has at most κ - 1 other nodes
    ...
    m.cons_fixnum_gates = pyo.Constraint(expr=(sum(m.Bg.values())
                                               == math.ceil(N/κ)))
    ...
    #  m.cons_most1_gate = pyo.Constraint(
    #      m.pairs,
    #      rule=lambda m, u, v: m.Bg[u] + m.Bg[v] + m.Ba[u, v] <= 2)

    # Bng, Bg and Ba are linked
    def link_Bng_Bg_Ba(m, u, v):
        if u == v:
            return pyo.Constraint.Skip
        s, t = (u, v) if u < v else (v, u)
        return 2*m.Bng[u, v] <= m.Bg[v] + m.Ba[s, t]
    m.cons_link_Bng_Bg_Ba = pyo.Constraint(m.N, m.N, rule=link_Bng_Bg_Ba)

    # enforce one gate per node
    m.cons_single_gate = pyo.Constraint(
        m.N, rule=lambda m, n: sum(m.Bng[n, nn] for nn in m.N) == 1)
    ...
    m.length = pyo.Objective(
        expr=lambda m: (pyo.sum_product(m.d, m.Be)
                        + pyo.sum_product(m.g, m.Bg, index=m.N)),
        sense=pyo.minimize,
    )
