...
def make_min_length_model_balanced(A: nx.Graph, capacity: int, *,
                                   num_gates: int,
    ...
    assert math.ceil(T/capacity) <= num_gates, (
        f'Not enough gates (num_gates = {num_gates}) '
        f'for capacity = {capacity} and T = {T}'
    )
    gate_load_matched, gates_plus1 = divmod(T, num_gates)
    ...
    m.Bmatched = pyo.Var(m.R, m.T,
                         domain=pyo.Binary,
                         initialize=0)
    m.Bplus1 = pyo.Var(m.R, m.T,
                       domain=pyo.Binary,
                       initialize=0)
    ...
    m.cons_gates_matched = pyo.Constraint(
        rule=lambda m: (sum(m.Bmatched[r, u] for r in m.R for u in m.T)
                        == num_gates - gates_plus1)
    )
    m.cons_gates_plus1 = pyo.Constraint(
        rule=lambda m: (sum(m.Bplus1[r, u] for r in m.R for u in m.T)
                        == gates_plus1)
    )
    # assert all nodes are connected to some root
    m.cons_total_gates_load = pyo.Constraint(
        rule=lambda m: (sum(gate_load_matched*m.Bmatched[r, u]
                            for r in m.R for u in m.T)
                        + sum((gate_load_matched + 1)*m.Bplus1[r, u]
                              for r in m.R for u in m.T) == W)
    )

    # there is at most one gate edge per node
    m.cons_one_root = pyo.Constraint(
        m.T,
        rule=lambda m, n: sum(m.Bmatched[:, n]) + sum(m.Bplus1[:, n]) <= 1)
    ...
    m.cons_edges_eq_nodes = pyo.Constraint(
        rule=lambda m: (sum(m.Be[u, v] for u, v in m.diE)
                        + sum(m.Bmatched[r, n] + m.Bplus1[r, n]
                              for r in m.R for n in m.T) == T)
    )
    ...
    m.cons_flow_conservation = pyo.Constraint(
        m.T,
        rule=lambda m, u: (
            sum((m.De[u, v] - m.De[v, u]) for v in A_nodes.neighbors(u))
            + gate_load_matched*sum(m.Bmatched[r, u] for r in m.R)
            + (gate_load_matched + 1)*sum(m.Bplus1[r, u] for r in m.R)
    ...
    m.cons_one_out_edge = pyo.Constraint(
        ...
        rule=lambda m, u: (sum(m.Be[u, v] for v in A_nodes.neighbors(u))
                           + sum(m.Bmatched[r, u] for r in m.R)
                           + sum(m.Bplus1[r, u] for r in m.R) == 1)
    )
    ...
    m.length = pyo.Objective(
        expr=lambda m: (pyo.sum_product(m.d, m.Be)
                        + pyo.sum_product(m.g, m.Bmatched)
                        + pyo.sum_product(m.g, m.Bplus1)),
        sense=pyo.minimize,
    )
    ...
    m.gate_load_matched = gate_load_matched
...
def warmup_model_balanced(model: pyo.ConcreteModel, S: nx.Graph) \
        -> pyo.ConcreteModel:
    '''
    ...
    Ne = len(model.diE)//2
    T = len(model.T)
    num_gates = model.method_options['num_gates']
    gate_load_matched, gates_plus1 = divmod(T, num_gates)
    ...
    for r in model.R:
        for n in S.neighbors(r):
            ...
            if S[r][n]['load'] == gate_load_matched:
                model.Bmatched[r, n] = 1
            else:
                model.Bplus1[r, n] = 1
...
def S_from_solution_balanced(model: pyo.ConcreteModel, solver: SolverBase,
    ...
    if solver.name.endswith('direct'):
    ...
    S.add_weighted_edges_from(
        ((r, n, model.gate_load_matched)
         for (r, n), bg in model.Bmatched.items()
         if bg.value > 0.5),
        weight='load'
    )
    # gates
    S.add_weighted_edges_from(
        ((r, n, model.gate_load_matched + 1)
         for (r, n), bg in model.Bplus1.items()
         if bg.value > 0.5),
        weight='load'
    )
    # node-node edges
...
def gurobi_investigate_pool_balanced(P, A, model, solver, result):
    ...
    for i in range(num_solutions):
        ...
        S = S_from_solution_balanced(model, solver=solver, result=result)
        G = G_from_S(S, A)
