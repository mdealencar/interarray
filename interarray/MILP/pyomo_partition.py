...
def make_MILP_length(A, k, gateXings_constraint=False, gates_limit=False,
    ...
    #                 bounds=(0, m.k),
    #                 initialize=0)
    m.Bb = pyo.Var(m.branches, m.N, domain=pyo.Binary, initialize=0)
    ...
    m.cons_subtree_size = pyo.Constraint(
        m.branches, rule=lambda m, b: sum(m.Bb[b, n] for n in m.N) <= k)
    # TODO: if using the minumum number of branches, set a constraint for the
    # minimum subtree size as well

    # each node belongs to exactly one branch
    m.cons_nodes_in_single_subtree = pyo.Constraint(
        m.N, rule=lambda m, n: sum(m.Bb[b, n] for b in m.branches) == 1)

    # exactly one gate per branch
    # TODO: fix this constraint to account for empty branches
    m.cons_branches_rooted = pyo.Constraint(
        m.branches, rule=lambda m, b: sum(m.Brooted_by[b, n] for n in m.N) == 1)
    # node roots a branch iif it belongs to it
    m.cons_gate_in_branch = pyo.Constraint(
        m.N, m.branches, rule=lambda m, n, b: m.Bb[b, n] >= m.Brooted_by[b, n])

    # if edge ⟨u, v⟩ is active, then u and v belong to the same branch
    m.cons_same_branch = pyo.Constraint(
        m.E,
        #  m.E, m.branches,
        rule=lambda m, u, v: (m.Bb[b, u] + m.Bb[b, v]
