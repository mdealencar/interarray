...
def make_MILP_length(A, k, gateXings_constraint=False, gates_limit=False,
    ...
    m.together = pyo.Var(m.pairs, domain=pyo.Binary, initialize=0)
    ...
    #  m.Brooted_by = pyo.Var(m.pairs, domain=pyo.Binary, initialize=0)
    m.Bg = pyo.Var(m.N,
                   domain=pyo.Binary,
                   initialize=0)
    ...
    # each branch has at most k nodes ->
    # each node is together with at most k - 1 other nodes
    # if a node is not a gate, line/column is empty
    m.cons_subtree_size = pyo.Constraint(
        m.N,
        rule=lambda m, n: (sum(m.together[(n, u)] for u in range(n + 1, N))
                           + sum(m.together[(v, n)] for v in range(0, n))
                           #  <= (k - 1)))
                           <= (k - 2)*m.Bg[n] + 1))
    # TODO: if using the minimum number of branches, set a constraint for the
    # minimum subtree size as well

    # each node is together with a gate
    #  m.cons_together_with_gate = pyo.Constraint(
    #      m.N,
    #      rule=lambda m, n: (sum(m.together[(n, u)] for u in range(n + 1, N))
    #                         + sum(m.together[(v, n)] for v in range(0, n))
    #                         >= m.Bg[n]))
    m.cons_together_with_something = pyo.Constraint(
        m.N,
        rule=lambda m, n: (sum(m.together[(n, u)] for u in range(n + 1, N))
                           + sum(m.together[(v, n)] for v in range(0, n))
                           + m.Bg[n]
    ...
    # if ⟨u, v⟩ is active, some gate is together with both u and v
    #  m.cons_bind_edge_and_branch = pyo.Constraint(
    #      m.E, m.N,
    #      rule=lambda m, u, v, n: (m.together[(n, u) if n < u else (u, n)]
    #                               + m.together[(n, v) if n < v else (v, n)]
    #                               == 2*m.Be[u, v]*m.Bg[n]))
    m.cons_bind_edge_and_branch = pyo.Constraint(
        m.E,
        rule=lambda m, u, v: (sum((m.together[(n, u) if n < u else (u, n)]
                                  + m.together[(n, v) if n < v else (v, n)])
                                  for n in m.N
                                  if ((n != u) and (n != v)))
                              >= 2*m.Be[u, v]))
    ...
    # at least one edge or gate includes node
    m.cons_nodes_are_connected = pyo.Constraint(
        m.N,
        rule=lambda m, n: (sum(m.Be[(n, u) if n < u else (u, n)]
                               for u in A_nodes.neighbors(n))
                           + m.Bg[n] >= 1))

    # if a node is rooted by itself, its row/column is all zeros
    # this binds m.Brooted_by and m.Bg
    #  m.cons_one_gate_per_branch = pyo.Constraint(
    #      m.N,
    #      rule=lambda m, n: (sum(m.Brooted_by[(n, u)] for u in range(n + 1, N))
    #                         + sum(m.Brooted_by[(v, n)] for v in range(0, n))
    #                         + m.Bg[n]
    #                         == 1)
    #  )
    m.cons_minimum_gate_number = pyo.Constraint(
        rule=lambda m: sum(m.Bg.values()) >= math.ceil(N/m.k))

    # gates limit
    if gates_limit:
        def gates_limit_eq_rule(m):
            return (N - sum(m.Brooted_by.values())
                    == math.ceil(N/m.k))
        ...
        m.cons_gates_limit = pyo.Constraint(
            rule=(gates_limit_eq_rule
    ...
    m.cons_edges_eq_nodes = pyo.Constraint(
        rule=lambda m: (sum(m.Be.values())
                        + sum(m.Bg.values()) == N)
    )
    #  m.cons_edges_eq_nodes = pyo.Constraint(
    #      rule=lambda m: (sum(m.Be.values())
    #                      - sum(m.Brooted_by.values()) == 0)
    #  )
